...
bool ImGui::PlusButton(const char* str_id)
{
	...
	ImVec2 symSize = bb.GetSize() * 0.5f * 0.75f;
	ImVec2 center = bb.GetCenter();

	g.CurrentWindow->DrawList->AddRectFilled(center - ImVec2(symSize.x, symSize.y * 0.25f), center + ImVec2(symSize.x, symSize.y * 0.25f), GetColorU32(ImGuiCol_Text));
	g.CurrentWindow->DrawList->AddRectFilled(center - ImVec2(symSize.x * 0.25f, symSize.y), center + ImVec2(symSize.x* 0.25f, symSize.y ), GetColorU32(ImGuiCol_Text));

	return pressed;
}

bool ImGui::MinusButton(const char* str_id)
{
	ImGuiButtonFlags flags = 0;
	float sz = GetFrameHeight();
	ImVec2 size(sz, sz);
